...
function usage {
  echo "Usage: $0 [OPTION]..."
  echo "Run Arsenal's test suite(s)"
  echo ""
...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testropts" = "" ] && [ "$testrargs" = "" ]; then
      ...
      # provided.
      testrargs="discover ./arsenal/tests/"
    fi
  ...
  if [ setup.cfg -nt arsenal.egg-info/entry_points.txt ]
  then
  ...
  then
    ...
    bash -c "TESTS_DIR=$TESTS_DIR ${wrapper} $TESTRTESTS | ${wrapper} subunit-2to1 | ${wrapper} tools/colorizer.py"
  else
    bash -c "TESTS_DIR=$TESTS_DIR ${wrapper} $TESTRTESTS | ${wrapper} tools/colorizer.py"
  fi
  RESULT=$?
  set -e
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='arsenal/*' --omit='arsenal/openstack/common/*' -d covhtml -i
  fi
